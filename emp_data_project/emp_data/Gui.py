...
class emp_page(tk.Frame):
    ...
    def validate_employee(self, emp):
        ...
        if re.search(r"^\d\d\/\d\d\/\d\d\d\d$", emp['dob_entry']) is None \
            ...
            errorMsg['dob_err'] = '• Error: Dates must match the format: MM/DD/YYYY or MM-DD-YYYY\n'
        else:
        ...
        if emp['Pay_Method'] == 'Direct Deposit':
            if re.search(r"^\d+-?\d+$", emp['account_number_entry']) is None:
                ...
                if 'account_number_err' in errorMsg:
                    del errorMsg['account_number_err'] 
            ...
            if re.search(r"^\d+-?\d+-?\d+$", emp['routing_number_entry']) is None:
                ...
                if 'routing_number_err' in errorMsg:
                    del errorMsg['routing_number_err'] 
        ...
        if re.search(r"^\d\d\/\d\d\/\d\d\d\d$", emp['start_date_entry']) is None \
            ...
            errorMsg['start_date_err'] = '• Error: Start Date must match the format: MM/DD/YYYY or MM-DD-YYYY\n'
        else:
        ...
        if emp['end_date_entry']:  
            if re.search(r"^\d\d\/\d\d\/\d\d\d\d$", emp['end_date_entry']) is None \
                    ...
                    None:
                self.start_date_entry.config(highlightbackground = "red", highlightcolor= "red", highlightthickness=1)
                errorMsg['end_date_err'] = '• Error: End Date must match the format: MM/DD/YYYY or MM-DD-YYYY\n'
            else:
        ...
        if not emp['password_entry'].strip():
            ...
            if 'password_empty_err' in errorMsg:
                del errorMsg['password_empty_err']
    ...
    def make_new_employee(self):
        ...
        new_emp_val['ID'] = self.employee_selected.id
        ...
        if self.validate_employee(new_emp_val) is valid:
            if new_emp_val['Classification'] == 'Hourly':
                pass
                emp_class = create_classification(1,new_emp_val['hourly_entry'])
            elif new_emp_val['Classification'] == 'Salary':
                emp_class = create_classification(2,new_emp_val['salary_entry'])
            else:
                emp_class = create_classification(3,new_emp_val['commission_entry'])
              
            if new_emp_val['Pay_Method'] == 'Direct Deposit':
                pay_method_num = 1  
            elif new_emp_val['Pay_Method'] == 'Mail':
                pay_method_num = 2

            add_new_employee(EmpDat, new_emp_val['ID'], new_emp_val['first_name_entry'], new_emp_val['last_name_entry'],
                             new_emp_val['address_entry'], new_emp_val['city_entry'], new_emp_val['State'], new_emp_val['zip_entry'], emp_class,
                             pay_method_num, new_emp_val['dob_entry'], new_emp_val['ssn_entry'], new_emp_val['personal_phone_entry'], new_emp_val['personal_email_entry'],
                             new_emp_val['start_date_entry'], new_emp_val['job_title_entry'], new_emp_val['job_department_entry'], new_emp_val['permission'],
                             new_emp_val['password_entry'], new_emp_val['routing_number_entry'], new_emp_val['account_number_entry'])
            self.update()
