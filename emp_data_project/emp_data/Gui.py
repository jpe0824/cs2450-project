...
class emp_page(tk.Frame):
    ...
    def validate_employee(self, emp):
        ...
        if re.search(r"^\(\d\d\d\)\d\d\d-\d\d\d\d$", emp['personal_phone_entry']) is None \
            ...
            errorMsg['personal_phone_err'] = '• Error: Phone number must match one of the following formats: \n   (###)###-#### or ###-###-#### or ##########!\n'
        ...
        if self.hourly_entry.grid_info():
            try:             
                ...
                self.hourly_entry.config(highlightbackground = "red", highlightcolor= "red", highlightthickness=1)
        ...
        if self.salary_entry.grid_info():
            try:
                ...
                self.salary_entry.config(highlightbackground = "red", highlightcolor= "red", highlightthickness=1)
        ...
        if self.commission_entry.grid_info():
            try:
                ...
                self.commission_entry.config(highlightbackground = "red", highlightcolor= "red", highlightthickness=1)
        ...
        if re.search(r"^\d\d\d-\d\d-\d\d\d\d$", emp['ssn_entry']) is None and \
            ...
            errorMsg['ssn_err'] = '• Error: SSN must match the format: ###-##-#### or ######### (9 digits)\n'
        ...
        if emp['Pay_Method'] == 'Direct Deposit':
            if re.search(r"^\d+-?\d+$", emp['account_number_entry']) is None:
                self.account_number_entry.config(highlightbackground = "red", highlightcolor= "red", highlightthickness=1)
            ...
            if re.search(r"^\d+-?\d+-?\d+$", emp['routing_number_entry']) is None:
                self.routing_number_entry.config(highlightbackground = "red", highlightcolor= "red", highlightthickness=1)
        ...
        if emp['end_date_entry']:  
            if re.search("^[0-9]{1,2}\\/[0-9]{1,2}\\/[0-9]{4}$", emp['end_date_entry']) is None:
                self.end_date_entry.config(highlightbackground = "red", highlightcolor= "red", highlightthickness=1)
    ...
    def make_new_employee(self):
        ...
        if self.validate_employee(new_emp) is valid:
            if new_emp['Classification'] == 'Hourly':
                ...
                emp_class = create_classification(3,new_emp['salary_entry'], new_emp['commission_entry'])
            ...
            employee = add_new_employee(EmpDat, new_emp['ID'], new_emp['first_name_entry'], new_emp['last_name_entry'],
                             ...
                             pay_method_num, new_emp['dob_entry'], new_emp['ssn_entry'], new_emp['personal_phone_entry'], new_emp['personal_email_entry'],
                             new_emp['start_date_entry'], new_emp['job_title_entry'], new_emp['job_department_entry'], new_emp['permission'],
    ...
    def emp_page_entries(self, employee, mode = None):
        ...
        var = StringVar()
        var.set('None')
        emp_id = StringVar(self.view_frame, value=employee.id,name='emi_id_var')
        first_name = StringVar(self.view_frame, value=employee.first_name,name='first_name')
        last_name = StringVar(self.view_frame, value=employee.last_name,name='last_name_var')
        address = StringVar(self.view_frame, value=employee.address,name='address_var')
        city = StringVar(self.view_frame, value=employee.city,name='city_var')
        zipcode = StringVar(self.view_frame, value=employee.zip,name='zipcode_var')
        ssn = StringVar(self.view_frame, value=employee.ssn,name='ssn_var')
        start_date = StringVar(self.view_frame, value=employee.start_date,name='start_date_var')
        if not start_date.get():
            start_date.set('MM/DD/YYYY')
        end_date = StringVar(self.view_frame, value=employee.end_date,name='end_date_var')
        if not end_date.get():
            end_date.set('MM/DD/YYYY')
        dob = StringVar(self.view_frame, value=employee.birth_date,name='dob_var')
        if not dob.get():
            dob.set('MM/DD/YYYY')
        job_title = StringVar(self.view_frame, value=employee.title,name='job_title_var')
        job_dept = StringVar(self.view_frame, value=employee.dept,name='job_dept_var')
        phone = StringVar(self.view_frame, value=employee.phone,name='phone_var')
        email = StringVar(self.view_frame, value=employee.email,name='email_var')
        password = StringVar(self.view_frame, value=employee.password,name='password_var')
        job_status = StringVar(self.view_frame, value=employee.job_status,name='job_status_var')
        routingNum = StringVar(self.view_frame, value='',name='routingNum_var')
        accountNum = StringVar(self.view_frame, value='',name='accountNum_var')
        ...
        self.commission_label = tk.Label(self.view_frame, name='commission_label', text="Commission Rate:", font=('Arial', 10))
        self.commission_entry = tk.Entry(self.view_frame, name='commission_entry', state=DISABLED, textvariable=commission, font=('Arial', 10))
        ...
        def updateCol3():
            self.job_title_label.grid(column=3, row=4+self.row_shift, sticky='E', padx=15, pady=7)
            self.job_department_label.grid(column=3, row=5+self.row_shift, sticky='E', padx=15, pady=7)
            self.start_date_label.grid(column=3, row=6+self.row_shift, sticky='E', padx=15, pady=7)
            self.end_date_label.grid(column=3, row=7+self.row_shift, sticky='E', padx=15, pady=7)
            self.employment_status_label.grid(column=3, row=8+self.row_shift, sticky='E', padx=15, pady=7)
        ...
        def pay_method_dropdown_func(*args):
            if self.payment_method_clicked.get() == 'Mail':
                ...
                self.routing_number_label.grid(column=3, row=5, sticky='E', padx=15, pady=7)
        ...
        if mode != 'cancel':
            ...
            self.state_clicked = StringVar(self.view_frame, value=employee.state,name='')
            ...
            self.classification_clicked = StringVar(self.view_frame, value='',name='classification_clicked_val')
            ...
            self.payment_method_clicked = StringVar(self.view_frame, value='',name='payment_method_clicked_val')
            ...
            self.permission_level_clicked = StringVar(self.view_frame,value=employee.permission, name='permission_level_clicked_val')
        ...
        self.payment_method_label.grid(column=3, row=3, sticky='E', padx=15, pady=7)
...
class admin_page(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.searchEmp_Btn = Button(self, text="Search", font=('Arial', 10), command=lambda: self.search_for_emp()).place(x=710,y=420)
        ...
        self.addEmpBtn = tk.Button(self, text="Add New Employee", command= lambda: self.add_emp()).place(x=550,y=450)
        ...
        self.emp_tree = ttk.Treeview(self, columns=self.columns_list, show='headings')
        ...
        for col in self.columns_list:
            self.emp_tree.heading(col, 
                        command=lambda c=col: self.treeview_sort_column(self.emp_tree, c, False)
                        )
            #create heading
        ...
        self.emp_tree.heading("phone_number_column", text="Phone Number", anchor=W)
        ...
        self.emp_tree.heading("classification_column", text="Classification", anchor=W)
        self.emp_tree.heading("title_column", text="Title", anchor=W)
        self.emp_tree.heading("deptartment_column", text="Department", anchor=W)
    ...
    def treeview_sort_column(self, treeview, col, reverse):
        ...
        treeview.heading(
            col,
            command=lambda c=col: self.treeview_sort_column(treeview, c, not reverse)
        )
    
    
    # Iterate through all employees to list them out.
    
    def emp_tree_init(self): #initilizes employee tree, made function so it can be called later when needed for updates.
        global COUNT
        COUNT = 0
